...
/**
 * Keep Helm Code's `~/.helmcode/userdata/state.sqlite` projection in sync.
 ...
 * an unchanged state DB performs no writes and emits no websocket frames —
 * the same no-op property Codex's byte cursor gives its watcher path.
 */
function startHelmcodeSync(broadcast) {
  const fs = require("fs");
  const { getHelmcodeUserDataDir, onHelmcodeHomeChanged } = require("./lib/helmcode-home");
  const { syncHelmcodeSessions, reconcileHelmcodeLiveness } = require("./lib/helmcode-ingest");
  let running = false;
  let queued = false;
  let watcher = null;
