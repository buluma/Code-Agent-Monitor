/**
 * @file Verifies T3's best-effort cost attribution: reading and
 * flattening `usage-model-rates.json`, resolving a thread's model id through
 * T3's region/provider routing aliases, mtime-based rate-file caching,
...
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), "cam-t3-pricing-"));
process.env.DASHBOARD_T3_HOME = TMP;

const { loadT3ModelRates, resolveT3ModelRate, calculateT3Cost } = require("../lib/t3-pricing");
...
describe("T3 pricing", () => {
  ...
  it("returns null when the rate file has not been fetched yet", () => {
    assert.equal(loadT3ModelRates(), null);
  });
  ...
  it("resolves an exact model id before trying any prefix", () => {
    ...
    const rates = loadT3ModelRates();
    assert.deepEqual(resolveT3ModelRate(rates, "claude-sonnet-5"), {
      input: 2e-6,
      output: 1e-5,
    });
  });
  ...
  it("resolves a region/provider-routed model id by stripping the known prefix", () => {
    writeRates({ "claude-sonnet-5": { input_cost_per_token: 2e-6, output_cost_per_token: 1e-5 } });
    const rates = loadT3ModelRates();
    assert.deepEqual(resolveT3ModelRate(rates, "us.anthropic.claude-sonnet-5"), {
      input: 2e-6,
      output: 1e-5,
    });
    assert.deepEqual(resolveT3ModelRate(rates, "vertex_ai/claude-sonnet-5"), {
      input: 2e-6,
      output: 1e-5,
    });
  });
  ...
  it("returns null for a model with no matching entry under any known prefix", () => {
    writeRates({ "claude-sonnet-5": { input_cost_per_token: 2e-6, output_cost_per_token: 1e-5 } });
    const rates = loadT3ModelRates();
    assert.equal(resolveT3ModelRate(rates, "some-unknown-model"), null);
  });
  ...
  it("costs a priced bucket and leaves an unpriced one at zero with a reason", () => {
    ...
    assert.equal(result.breakdown[0].provider, "t3");
    ...
    assert.match(result.unpriced_models[0].reason, /No t3 rate entry/);
  });
