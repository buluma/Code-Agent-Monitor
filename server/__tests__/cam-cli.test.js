/**
 * @file Tests for the `cam` umbrella CLI (bin/cam.js). Spawns the real CLI
...
const STAMP = `cam-cli-${Date.now()}-${process.pid}`;
...
before(async () => {
  ...
  const code = await post("/api/hooks/event", {
    hook_type: "Stop",
    data: { session_id: "cli-test-session-0001", cwd: "/tmp/cam-cli" },
  });
...
describe("cam CLI — monitoring", () => {
  it("health reports the dashboard as up", async () => {
    const { code, out } = await cam("health");
    assert.equal(code, 0);
  ...
  it("stats prints totals including the seeded session", async () => {
    const { code, out } = await cam("stats");
    assert.equal(code, 0);
  ...
  it("kanban groups sessions and agents into status columns", async () => {
    const { code, out } = await cam("kanban");
    assert.equal(code, 0);
...
describe("cam CLI — data browsing", () => {
  it("sessions lists the seeded session", async () => {
    const { code, out } = await cam("sessions", "--limit", "5");
    assert.equal(code, 0);
  ...
  it("sessions --status filters correctly", async () => {
    const { code, out } = await cam("sessions", "--status", "completed");
    assert.equal(code, 0);
    assert.match(out, /of 0 session/);
  });

  it("session <id> shows detail with agents and recent events", async () => {
    const { code, out } = await cam("session", "cli-test-session-0001");
    assert.equal(code, 0);
  ...
  it("session without an id exits 1 with usage", async () => {
    const { code, err } = await cam("session");
    assert.equal(code, 1);
    assert.match(err, /Usage: cam session/);
  });

  it("agents lists the auto-created main agent", async () => {
    const { code, out } = await cam("agents", "--session", "cli-test-session-0001");
    assert.equal(code, 0);
    assert.match(out, /main/);
  });

  it("events lists the seeded Stop event", async () => {
    const { code, out } = await cam("events", "--session", "cli-test-session-0001");
    assert.equal(code, 0);
    assert.match(out, /Stop/);
  });

  it("transcript reads the provider-aware transcript endpoint", async () => {
    const { code, out } = await cam("transcript", "cli-test-session-0001", "--limit", "5");
    assert.equal(code, 0);
...
describe("cam CLI — insights", () => {
  it("analytics prints token totals and averages", async () => {
    const { code, out } = await cam("analytics");
    assert.equal(code, 0);
  ...
  it("workflows prints intelligence stats", async () => {
    const { code, out } = await cam("workflows");
    assert.equal(code, 0);
  ...
  it("run list exposes the live Run API", async () => {
    const { code, out } = await cam("run", "list");
    assert.equal(code, 0);
    const parsed = JSON.parse(out);
  ...
  it("cost warns about models with usage but no pricing rule", async () => {
    ...
    ).run("cli-test-session-0001", "cam-mystery-model-9", 1200, 300);
    try {
      const { code, out } = await cam("cost");
      assert.equal(code, 0);
      assert.match(out, /no pricing rule/);
      assert.match(out, /cam-mystery-model-9/);
      assert.match(out, /cam pricing set/);
    } finally {
      db.prepare("DELETE FROM token_usage WHERE model = 'cam-mystery-model-9'").run();
    }
  });
});

describe("cam CLI — alerts, rules, webhooks", () => {
  it("alerts lists the (empty) fired-alert feed", async () => {
    const { code, out } = await cam("alerts");
    assert.equal(code, 0);
    assert.match(out, /unacknowledged of/);
  });

  it("alerts ack-all succeeds on an empty feed", async () => {
    const { code, out } = await cam("alerts", "ack-all");
    assert.equal(code, 0);
    assert.match(out, /Acknowledged/);
  });

  it("rules lists alert rules", async () => {
    const { code, out } = await cam("rules");
    assert.equal(code, 0);
    assert.match(out, /Enabled/);
  });

  it("webhooks lists targets", async () => {
    const { code, out } = await cam("webhooks");
    assert.equal(code, 0);
    assert.match(out, /Provider/);
  });

  it("webhooks providers exposes the provider catalog", async () => {
    const { code, out } = await cam("webhooks", "providers");
    assert.equal(code, 0);
    const parsed = JSON.parse(out);
  ...
  it("webhook update and delete require an id", async () => {
    for (const action of ["update", "delete"]) {
      const result = await cam("webhooks", action, "--yes");
  ...
  it("unknown alert-rule subcommands report usage before confirmation", async () => {
    const result = await cam("alert-rules", "unknown");
    assert.equal(result.code, 1);
    assert.match(result.err, /Usage: cam alert-rules/);
    assert.doesNotMatch(result.err, /require --yes/);
  });
});

describe("cam CLI — remote sources", () => {
  it("adds and lists an independent remote Codex home", async () => {
    const added = await cam(
      "remote-sources",
      "add",
      "--label",
      "Codex build host",
    ...
    const listed = await cam("remote-sources");
...
describe("cam CLI — pricing", () => {
  it("pricing lists the default rules", async () => {
    const { code, out } = await cam("pricing");
    assert.equal(code, 0);
  ...
  it("pricing set / delete round-trips a custom rule", async () => {
    ...
    const list = await cam("pricing");
    assert.match(list.out, /cam-test-model%/);
    const del = await cam("pricing", "delete", "cam-test-model%");
  ...
  it("pricing set persists fast-mode and intro rates via flags", async () => {
    ...
    assert.equal(set.code, 0, `stderr: ${set.err} stdout: ${set.out}`);
    const list = await cam("pricing");
    assert.match(list.out, /cam-fast-model%/);
    ...
    const edit = await cam("pricing", "set", "cam-fast-model%", "--input", "6", "--output", "30");
    assert.equal(edit.code, 0);
    const after = await cam("pricing");
    assert.match(after.out, /\$6/);
    assert.match(after.out, /2099-01-01/);
    await cam("pricing", "delete", "cam-fast-model%");
  });
});

describe("cam CLI — import & administration", () => {
  it("import guide supports Codex", async () => {
    const { code, out } = await cam("import", "guide", "--provider", "codex");
    assert.equal(code, 0);
    const parsed = JSON.parse(out);
  ...
  it("config claude overview exposes the Config Explorer API", async () => {
    const { code, out } = await cam("config", "claude", "overview");
    assert.equal(code, 0);
    const parsed = JSON.parse(out);
  ...
  it("hooks status exposes both providers", async () => {
    const { code, out } = await cam("hooks", "status");
    assert.equal(code, 0);
    const parsed = JSON.parse(out);
  ...
  it("api GET provides complete low-level read coverage", async () => {
    const { code, out } = await cam("api", "GET", "/api/health");
    assert.equal(code, 0);
    const parsed = JSON.parse(out);
  ...
  it("api GET preserves non-JSON responses such as Prometheus metrics", async () => {
    const { code, out } = await cam("api", "GET", "/api/metrics");
    assert.equal(code, 0);
    assert.match(out, /^# HELP cam_up/m);
  });

  it("api writes require --yes", async () => {
    const { code, err } = await cam("api", "POST", "/api/alerts/ack-all");
    assert.equal(code, 1);
    assert.match(err, /require --yes/);
  });

  it("generic clear-data requires the exact confirmation token", async () => {
    const { code, err } = await cam("api", "POST", "/api/settings/clear-data", "--yes");
    assert.equal(code, 1);
  ...
  it("export writes a JSON file containing the seeded session", async () => {
    ...
    const { code, out } = await cam("export", file);
    assert.equal(code, 0);
  ...
  it("import-data round-trips an export through the server restore route", async () => {
    ...
    const exported = await cam("export", file);
    assert.equal(exported.code, 0);
    const restored = await cam("import-data", file);
  ...
  it("import upload sends multipart history files through the CLI", async () => {
    ...
    const uploaded = await cam("import", "upload", file, "--provider", "claude");
  ...
  it("update-check reports the checkout's update status", async () => {
    ...
    const { code, out } = await cam("update-check");
    assert.equal(code, 0);
  ...
  it("cleanup without flags exits 1 with usage", async () => {
    const { code, err } = await cam("cleanup");
    assert.equal(code, 1);
    assert.match(err, /Usage: cam cleanup/);
  });

  it("cleanup with flags runs", async () => {
    const { code, out } = await cam("cleanup", "--days", "3650");
    assert.equal(code, 0);
    assert.match(out, /Cleanup done/);
  });

  it("clear-data REFUSES without --yes", async () => {
    const { code, err } = await cam("clear-data");
    assert.equal(code, 1);
    assert.match(err, /--yes/);
    // Data must be intact.
    const { out } = await cam("sessions");
    assert.match(out, /of 1 session/);
  });

  it("clear-data --yes wipes rows (runs last)", async () => {
    const { code, out } = await cam("clear-data", "--yes");
    assert.equal(code, 0);
    assert.match(out, /cleared/);
    const { out: after } = await cam("sessions");
    assert.match(after, /of 0 session/);
  });
});

describe("cam CLI — offline mode (server down, DB read directly)", () => {
  ...
  before(async () => {
    const code = await post("/api/hooks/event", {
      hook_type: "Stop",
      data: { session_id: "cli-test-offline-0002", cwd: "/tmp/cam-cli-off" },
    });
    assert.equal(code, 200);
  });
  ...
  it("corrects dead active sessions display-side when the probe can answer", async (t) => {
    ...
    // (/tmp/cam-cli-off) has no running claude process, so its stored
...
describe("cam CLI — help & errors", () => {
  it("help lists every command group", async () => {
    const { code, out } = await cam("help");
    assert.equal(code, 0);
    for (const word of [
      "status",
      "start",
  ...
  it("no arguments prints help and exits 0", async () => {
    const { code, out } = await cam();
    assert.equal(code, 0);
    assert.match(out, /Usage: cam <command>/);
  });
  ...
  it("version prints the package version", async () => {
    ...
    const { code, out } = await cam("version");
    assert.equal(code, 0);
    assert.equal(out.trim(), `cam ${pkg.version}`);
    const flag = await cam("--version");
    assert.equal(flag.out.trim(), `cam ${pkg.version}`);
  });

  it("--no-color is accepted anywhere on the command line", async () => {
    const before = await cam("--no-color", "health");
    assert.equal(before.code, 0);
    assert.match(before.out, /Dashboard/);
    const after = await cam("sessions", "--no-color");
    assert.equal(after.code, 0);
    assert.match(after.out, /of 1 session/);
  });

  it("piped output contains no ANSI escape codes (colors off when not a TTY)", async () => {
    const { out } = await cam("stats");
  ...
  it("unknown command exits 1 with an error", async () => {
    const { code, err } = await cam("frobnicate");
    assert.equal(code, 1);
  ...
  it("status exits 1 with the down indicator when the server is down", async () => {
    ...
    assert.match(r.out, /cam start/);
  });

  it("start no-ops with a pointer when a server is already running", async () => {
    const { code, out } = await cam("start");
    assert.equal(code, 0);
    assert.match(out, /already running/);
  });
});

describe("cam CLI — interactive REPL", () => {
  /**
   * Drive `cam repl` with piped stdin (non-TTY). Each typed line is executed
   * as a child `cam` process, so grandchildren reach the in-test server via
