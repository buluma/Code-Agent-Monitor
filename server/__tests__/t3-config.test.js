/**
 * @file Verifies the T3 Config Explorer HTTP surface end-to-end. The
...
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), "cam-t3-config-"));
...
const router = require("../routes/t3-config");
...
describe("T3 Config Explorer route", () => {
  let server;
  let port;
  ...
  describe("with a populated state.db fixture", () => {
    it("GET /overview reports home, state DB, runtime, and projection counts", async () => {
      // The dashboard DB already has the t3 stmts loaded via the require above.
      ...
      const body = res.json;
      assert.equal(body.home, process.env.DASHBOARD_T3_HOME);
      ...
      assert.equal(body.env.DASHBOARD_T3_HOME, process.env.DASHBOARD_T3_HOME);
    ...
    it("POST /resync without confirmation returns 400 ENOTCONFIRMED", async () => {
      const res = await request(port, "POST", "/api/t3-config/resync", {});
    ...
    it("POST /resync with confirmation returns ok + summary and materializes sessions", async () => {
      const before = stmts.listT3Sessions.all().length;
      const res = await request(port, "POST", "/api/t3-config/resync", { confirmed: true });
      assert.equal(res.status, 200);
      ...
      const after = stmts.listT3Sessions.all().length;
  ...
  describe("with no state database (T3 never ran on this machine)", () => {
    it("GET /overview still returns 200 with projection_counts: null and state_db.exists: false", async () => {
      ...
      const emptyHome = fs.mkdtempSync(path.join(os.tmpdir(), "cam-t3-empty-"));
      const saved = process.env.DASHBOARD_T3_HOME;
      process.env.DASHBOARD_T3_HOME = emptyHome;
      try {
        if (server) {
          server.close();
          server = null;
        ...
        process.env.DASHBOARD_T3_HOME = saved;
      }
    });
  });
});
