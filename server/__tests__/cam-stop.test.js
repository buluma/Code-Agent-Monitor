/**
 * @file Tests for the `cam stop` CLI command. Verifies: server not running
...
const STAMP = `cam-stop-${Date.now()}-${process.pid}`;
...
describe("cam stop — server not running", () => {
  beforeEach(() => {
  ...
  it("reports nothing to stop when server is not reachable", async () => {
    const { code, out } = await camStop({ DASHBOARD_PORT: "1" });
    assert.equal(code, 0);
    assert.match(out, /not running|nothing to stop/i);
  });
});

describe("cam stop — graceful shutdown", () => {
  let target;
  ...
  it("stops the server gracefully via SIGTERM", async () => {
    const { code, out } = await camStop({ DASHBOARD_PORT: String(target.port) });
    assert.equal(code, 0);
...
describe("cam stop — SIGKILL escalation", () => {
...
describe("cam stop — stale PID", () => {
  ...
  it("exits 1 when the PID in discovery is not running", async () => {
    const { code, out, err } = await camStop({ DASHBOARD_PORT: String(target.port) });
    assert.equal(code, 1);
    assert.match(out + err, /not running|stale/i);
  });
});

describe("cam stop — missing discovery file", () => {
  let target;
