...
/**
 * @file cam — the Code Agent Monitor command-line interface.
 *
 ...
 * directly from any shell as `cam <command>`, or interactively at the
 * `cam ›` prompt via `cam repl`.
 *
 ...
 *
 * The REPL (`cam repl`, aliases `shell` / `i`) runs each entered line as a
 * short-lived child `cam` process, so its behavior is identical to the
...
// works both from a checkout (./bin/cam.js) and through the global symlink
...
// line, and force-enabled by FORCE_COLOR / CAM_COLOR=1 (useful for `watch`
...
const useColor = (() => {
  ...
  if (process.env.CAM_COLOR === "1") return true;
...
async function api(method, pathname, body) {
  ...
  let res;
  const token = process.env.DASHBOARD_API_TOKEN || process.env.CAM_API_TOKEN;
  try {
    res = await fetch(url, {
      method,
      headers: {
 ...
 * identical everywhere: the cam CLI talks to the local dashboard server,
 * and `cam start` (or npm run dev / npm start) brings one up.
 */
function serverDownExit(reason) {
  ...
  console.error(
    `    ${c.bold("cam start")}        ${c.dim("# production server in the background")}`
  );
  console.error(
...
async function cmdSession(positional) {
  ...
  if (!id) {
    console.error(c.red("✖ Usage: cam session <session-id>"));
    process.exit(1);
  }
...
async function cmdTranscript(flags, positional) {
  ...
  if (!sessionId) {
    console.error(c.red("✖ Usage: cam transcript <session-id> [options]"));
    process.exit(1);
  }
...
async function cmdTranscriptImage(flags, positional) {
  ...
  if (!sessionId || flags.line === undefined || flags.index === undefined) {
    console.error(
      c.red("✖ Usage: cam transcript-image <session-id> --line N --index N [--output image.png]")
    );
    process.exit(1);
  }
  const query = new URLSearchParams({
...
async function cmdRun(flags, positional) {
  ...
  console.error(
    c.red("✖ Usage: cam run list|history|models|binary|cwds|files|get|start|send|stop")
  );
  process.exit(1);
}
...
async function cmdCost(flags) {
  ...
  if (unpriced.length) {
    ...
    console.log(c.dim("  Add a rule with: cam pricing set <pattern> --input N --output N"));
  }
}
...
async function cmdAlertRules(flags, positional) {
  ...
  if (!writeSubcommands.has(sub)) {
    console.error(c.red("✖ Usage: cam alert-rules list|create|update <id>|delete <id>"));
    process.exit(1);
  }
  if (!flags.yes) {
...
/**
 * `cam remote-sources [list|add|test|sync|rm]` — manage the SSH machines this
...
async function cmdRemoteSources(flags, positional) {
  ...
  if (sub === "add") {
    if (!flags.label || !flags.host) {
      ...
      console.error(
        c.dim(
          "  e.g. cam remote-sources add --label 'Dev box' --host son@dev --port 22 --identity ~/.ssh/id_ed25519"
        )
      );
      process.exit(1);
    }
  ...
  if (sources.length > 0) {
    ...
    console.log(c.dim("  No remote sources configured. Add one: cam remote-sources add --help"));
  }
}
...
async function cmdImport(flags, positional) {
  ...
  console.error(
    c.red("✖ Usage: cam import guide|rescan|path <dir>|upload <files...> [--provider claude|codex]")
  );
  process.exit(1);
}
...
async function cmdConfig(flags, positional) {
  ...
  console.error(c.red("✖ Usage: cam config claude|codex|helmcode <action> [options]"));
  process.exit(1);
}
...
async function cmdHooks(flags, positional) {
  ...
  console.error(c.red("✖ Usage: cam hooks status | cam hooks install claude codex --yes"));
  process.exit(1);
}
...
async function cmdApi(flags, positional) {
  ...
  if (!pathname || !pathname.startsWith("/api/")) {
    console.error(c.red("✖ Usage: cam api <METHOD> /api/<path> [--data JSON|--file path]"));
    process.exit(1);
  }
...
async function cmdExport(positional) {
  const file = positional[0] || `cam-export-${new Date().toISOString().slice(0, 10)}.json`;
...
// Restore a bundle produced by `cam export` (or the dashboard's Export data).
...
async function cmdImportData(positional) {
  ...
  if (!target) {
    console.error(c.red("✖ Usage: cam import-data <export.json>"));
    process.exit(1);
  }
...
async function cmdCleanup(flags) {
  ...
  if (!flags.hours && !flags.days) {
    console.error(c.red("✖ Usage: cam cleanup --hours <N> and/or --days <M>"));
...
async function cmdClearData(flags) {
  if (flags.yes !== true) {
    ...
    console.error(c.dim("  Re-run with --yes to confirm: cam clear-data --yes"));
    process.exit(1);
  }
...
async function cmdMcp(positional) {
  ...
  if (mode === "http") args.push("--transport=http");
    ...
    console.error(c.red("✖ Usage: cam mcp [stdio|http|repl]"));
    process.exit(1);
  }
 ...
 * its output appended to data/cam-server.log, so closing this terminal does
...
async function cmdStart(flags) {
  ...
  if (!fs.existsSync(clientDist)) {
    ...
    console.error(c.dim("  Build it once with: npm run build   (then re-run: cam start)"));
    console.error(c.dim("  Or run the dev servers instead:     npm run dev"));
    process.exit(1);
  }
  const logDir = path.join(REPO_ROOT, "data");
  fs.mkdirSync(logDir, { recursive: true });
  const logFile = path.join(logDir, "cam-server.log");
...
async function cmdStatus() {
  if (await serverIsUp()) {
    ...
    console.log(c.dim("  Start it with: cam start   (or npm run dev / npm start)"));
    process.exit(1);
  }
}
...
const COMMAND_GROUPS = [
  ...
  [
    ...
    [
      ...
      ["version", "", "Print the cam version (also: --version, -v)"],
    ],
  ],
];
...
Disable with --no-color or NO_COLOR=1; force with FORCE_COLOR=1 / CAM_COLOR=1.

${c.bold("Note:")} cam talks to the local dashboard server — API-backed commands
require it to be running (${c.bold("cam start")} brings one up in the background).
Prefer an interactive session? Run ${c.bold("cam repl")} for a shell with completion,
...
function cmdHelp() {
  ...
  console.log(
    `${c.cyan("▍")}${c.bold("cam")} — Code Agent Monitor CLI\n\n` +
      `${c.bold("Usage:")} cam <command> [options]\n` +
      body +
      "\n" +
      helpFooter()
  );
...
function cmdVersion() {
  const v = pkgVersion();
  console.log(v ? `cam ${v}` : "cam (version unknown)");
}
...
const OFFLINE_HANDLERS = {
  ...
  async doctor() {
    heading("cam doctor", "offline");
    let failed = true; // offline is always a failure for live monitoring
    console.log(
      `${c.red("○")}  Dashboard server  NOT running ${c.dim(`(tried ${baseUrl()})`)} — start with: ${c.bold("cam start")}`
    );
    console.log(
      `${c.dim("·")}  Remote sources  require a live server (cam remote-sources / Settings)`
    );
    const file = dbPath();
...
function requireDb() {
  ...
  if (!db) {
    ...
    console.error(c.dim("  Start the server to begin capturing: cam start"));
    process.exit(1);
  }
  return db;
}

// ── Interactive REPL (cam shell) ───────────────────────────────────────────
// `cam repl` opens a persistent prompt where commands are typed WITHOUT the
// `cam` prefix. Each command runs as a short-lived child `cam` process, so
...
// The CAM word-mark, shown once when the interactive shell starts (and via
...
function replBanner(up) {
  ...
  console.log(
    c.dim("  Type commands without the 'cam' prefix — e.g. ") + c.bold("sessions --limit 5")
  );
  console.log(
    c.dim("  ") +
      c.bold("help") +
 ...
 *  catalog (identical groups/descriptions to `cam help`). */
function replHelp() {
  const row = (name, desc) => `  ${c.cyan(name.padEnd(20))}${desc}`;
  console.log(`\n${c.cyan("▍")}${c.bold("cam shell")} ${c.dim("— built-ins")}`);
...
/**
 * The interactive shell. Reads lines, dispatches each as a child `cam`
...
async function cmdRepl() {
  ...
  const historyFile = path.join(REPO_ROOT, "data", ".cam_repl_history");
  ...
  async function renderPrompt() {
    ...
    rl.setPrompt(`${dot} ${c.bold("cam")} ${where} ${c.cyan("›")} `);
    rl.prompt();
  }
  ...
  /** Run one child `cam` invocation with inherited stdio. */
  ...
  async function runWatch(argv, intervalMs) {
    ...
    try {
      while (!abort) {
        ...
        console.log(
          c.dim(
            `⟳ watch: cam ${argv.join(" ")} — every ${Math.round(intervalMs / 1000)}s · ${new Date().toLocaleTimeString()} · Ctrl+C to stop`
          )
        );
        await runChild(argv);
        if (abort) break;
  ...
  for await (const raw of rl) {
    ...
    if (head === "repl" || head === "shell" || head === "i") {
      console.log(c.dim("Already in the cam shell."));
      await renderPrompt();
      continue;
    }
...
// behaves identically whether typed as `cam <cmd>` or entered at the `cam ›`
