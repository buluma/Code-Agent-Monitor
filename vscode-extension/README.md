# Code Agent Monitor for VS Code

**Code Agent Monitor** brings a high-fidelity, real-time dashboard for your code
agent sessions directly into your VS Code workspace. Monitor token usage, track
agent health, and view deep analytics without switching windows.
...
Open the full agent monitoring dashboard in a dedicated VS Code tab. Supports
real-time updates via WebSockets and local-first data storage.
...
- **Token Tracking**: Real-time display of Billions (B) or Millions (M) of
  tokens consumed.
- **Cost Calculation**: Live estimate of total USD cost based on current model
  pricing.
- **Event Frequency**: High-signal stats on events per session and subagent
  spawning.
...
Ensure you have the Code Agent Monitor server running locally:
...
- Search for **"Code Agent Monitor"** in the VS Code Marketplace.
- Click **Install**.

### 3. Usage

- Click the **Radar/Dashboard icon** in the Activity Bar.
- The extension automatically detects your local server on ports **5173** (dev)
  or **4820** (prod).
...
3. Run the package command:
   ...
   vsce package
   ```
   _This will generate `code-agent-monitor-1.0.0.vsix`._
