...
| `typecheck:server`  | `npm run typecheck:server`  | Gradual JS type-check via `server/tsconfig.json` — only files opting in with a `// @ts-check` pragma are diagnosed                                                                                                            |
...
| `test:coverage:server` | `npm run test:coverage:server` | Server tests with `c8` coverage report (`coverage/`) — report-only, no fail threshold                                                                                                                                     |
| `test:coverage:client` | `npm run test:coverage:client` | Client tests with Vitest's built-in coverage report (`client/coverage/`) — report-only, no fail threshold                                                                                                                 |
