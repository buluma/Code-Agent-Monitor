...
end/close, or the process being re-parented away from the parent it started
under (polled every 5s). This prevents the orphaned, unreachable servers that a
host crashing without `SIGTERM`/`SIGINT` would otherwise leave behind.
Re-parenting is measured against the startup parent rather than pid 1, so a
server launched under an init-like parent (a container running `tini` as PID 1)
is not killed off as a false positive. See `docs/MCP.md` for the full lifecycle.

An HTTP session is tracked under the `mcp-session-id` returned by `initialize`
(or the `sessionId` handed to a legacy SSE client) and released on
`DELETE /mcp`, on SSE disconnect, or by an idle sweep — terminating a session is
optional in the protocol, so a client that crashes would otherwise pin its
`McpServer` forever. `GET /health` reports the live count as `activeSessions`;
tune with `MCP_HTTP_SESSION_TIMEOUT_MS`.
