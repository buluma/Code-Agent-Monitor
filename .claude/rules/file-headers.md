...
- Every applicable source file (`.js/.ts/.tsx/.cjs/.mjs/.py/.sh/.css` — excluding `node_modules/`, `dist/`, `data/`, `.worktrees/`, minified/vendored, snapshots) MUST start with the header comment: a truthful file overview plus the exact line `@author Michael Buluma <1452922+buluma@users.noreply.github.com>`.
