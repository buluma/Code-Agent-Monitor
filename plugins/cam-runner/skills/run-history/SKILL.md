...
name: run-history
description: >
  Inspect CAM dashboard-run history and relate live run handles to persisted
...
1. Run `cam run history --limit 100`.
2. Filter by provider, working directory, model, status, and session ID.
3. For a live handle, run `cam run get <id> --envelopes`.
4. For the persisted agent session, use `cam session <session-id>` and the
