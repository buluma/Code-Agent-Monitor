...
description: >
  Import Claude Code or Codex history and move complete CAM datasets between
...
1. Read provider-specific paths and limits:
   `cam import guide --provider claude|codex`.
2. Rescan the configured provider home:
   `cam import rescan --provider <provider>`.
3. Scan an existing directory:
   `cam import path /absolute/path --provider <provider>`.
...
```bash
cam export cam-export.json
cam import-data cam-export.json
```
