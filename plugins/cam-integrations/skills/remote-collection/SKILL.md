...
description: >
  Configure and troubleshoot CAM Remote Data Sources that collect Claude Code
...
1. List current sources with `cam remote-sources`.
2. Validate the host through the user's existing SSH configuration. CAM stores
...
4. Test before syncing: `cam remote-sources test <id>`.
5. Sync one source or all sources with `cam remote-sources sync [id]`.
6. Use `cam remote-sources update <id> --file patch.json --yes` for edits.
7. Removal retains imported sessions by default. Purging them requires:

```bash
cam remote-sources rm <id> --purge \
