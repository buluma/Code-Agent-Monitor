...
duplication, while bounded tail parses remain append-only.
The Helm Code mirror runs on the same continuous-background-sync tier: `server/index.js` starts `startHelmcodeSync`, which pairs an `fs.watch` on the state database (`state.sqlite` / `state.sqlite-wal`, re-registered whenever the file is replaced) with a DASHBOARD_HELMCODE_SYNC_MS safety-net poll (default 4000 ms, `0` disables). The watcher triggers the incremental sweep, which differs from the Codex path in one important way — it works from an **orchestration event log** (each thread's `THREAD_CREATED`/`THREAD_UPDATED` rows), so upstream events publish **after** the rows exist and the extracted `last_turn_row` advances monotonically. Helm Code installs no dashboard hooks; its cards are reconciled by the sweep alone.
